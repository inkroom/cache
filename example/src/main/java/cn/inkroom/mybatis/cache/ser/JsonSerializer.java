package cn.inkroom.mybatis.cache.ser;
...
public class JsonSerializer implements RedisSerializer<Object> {
    ...
    public Object deserialize(byte[] bytes) throws SerializationException {
        ...
        try {
            ...
            JavaType type = typeMap.get(Arrays.toString(cl));
            if (type != null) {
                return mapper.readValue(temp.substring(temp.indexOf(separator) + 1), type);
            }
            TypeFactory factory = mapper.getTypeFactory();
            if (cl.length == 1) {
//                return JSON.parseObject(temp.substring(temp.indexOf(separator) + 1), Class.forName(cl[0]));
                return mapper.readValue(temp.substring(temp.indexOf(separator) + 1), Class.forName(cl[0]));
            } else if (cl.length == 2) {//list

                type = factory.constructParametricType(Class.forName(cl[0]), Class.forName(cl[1]));
//                JSON.parseObject(temp.substring(temp.indexOf(separator) + 1),  new TypeReference<>(),null);
            } else if (cl.length == 3) {
                type = factory.constructParametricType(Class.forName(cl[0]), Class.forName(cl[1]), Class.forName(cl[2]));
            }
            typeMap.put(Arrays.toString(cl), type);
